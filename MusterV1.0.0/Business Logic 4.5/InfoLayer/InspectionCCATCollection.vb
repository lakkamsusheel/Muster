'-------------------------------------------------------------------------------
' MUSTER.Info.InspectionCCATCollection
...
Namespace MUSTER.Info
    Public Class InspectionCCATCollection
        Inherits DictionaryBase
        '#Region "Public Events"
        '        Public Event evtInspectionCCATColChanged(ByVal bolValue As Boolean)
        '#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.InspectionCCATInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.InspectionCCATInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.InspectionCCATInfo)
                If Not MyBase.Dictionary.Contains(Integer.Parse(index)) Then
                    MyBase.Dictionary.Add(Value.ID.ToString, Value)
                    'RaiseEvent evtInspectionCCATColChanged(True)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.InspectionCCATInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.InspectionCCATInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.InspectionCCATInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCCATInfo)) Then
                Throw New ArgumentException("Only InspectionCCATs may be inserted into a InspectionCCAT collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnRemove(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCCATInfo)) Then
                Throw New ArgumentException("Only InspectionCCATs may be removed from a InspectionCCAT collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            If Not newvalue.GetType() Is (GetType(MUSTER.Info.InspectionCCATInfo)) Then
                Throw New ArgumentException("Only InspectionCCATs updated in a InspectionCCAT collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnValidate(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCCATInfo)) Then
                Throw New ArgumentException("Only InspectionCCATs may be validated in a InspectionCCAT collection!", "value")
            End If
        End Sub
#End Region
    End Class
