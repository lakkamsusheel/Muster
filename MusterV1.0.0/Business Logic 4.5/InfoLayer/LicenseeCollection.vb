'-------------------------------------------------------------------------------
' MUSTER.Info.LicenseeCollection
...
'  1.0      RAF         05/24/2005      Original class definition
'
...
'
' NOTE: This file to be used as Licensee to build other objects.
'       Replace keyword "Licensee" with respective Object name.
'       Also - don't forget to update the revision history above!!!
'-------------------------------------------------------------------------------
Namespace MUSTER.Info
    Public Class LicenseeCollection
        Inherits DictionaryBase
#Region "Public Events"
        Public Event LicenseeColChanged(ByVal bolValue As Boolean)
#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.LicenseeInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.LicenseeInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.LicenseeInfo)
                If Not MyBase.Dictionary.Contains(index) Then
                    MyBase.Dictionary.Add(Value.ID.ToString, Value)
                    RaiseEvent LicenseeColChanged(True)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.LicenseeInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.LicenseeInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.LicenseeInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.LicenseeInfo)) Then
                Throw New ArgumentException("Only Licensee may be inserted into a Licensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnRemove(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.LicenseeInfo)) Then
                Throw New ArgumentException("Only Licensee may be removed from a Licensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            If Not newvalue.GetType() Is (GetType(MUSTER.Info.LicenseeInfo)) Then
                Throw New ArgumentException("Only Licensee updated in a Licensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnValidate(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.LicenseeInfo)) Then
                Throw New ArgumentException("Only Licensee may be validated in a Licensee collection!", "value")
            End If
        End Sub
#End Region
    End Class
