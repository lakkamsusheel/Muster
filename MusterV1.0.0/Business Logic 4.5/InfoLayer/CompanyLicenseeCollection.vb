'-------------------------------------------------------------------------------
' MUSTER.Info.CompanyLicenseeCollection
...
'  1.0        RAF       05/17/2005                      Original class definition
'
...
'
' NOTE: This file to be used as CompanyLicensee to build other objects.
'       Replace keyword "CompanyLicensee" with respective Object name.
'       Also - don't forget to update the revision history above!!!
'-------------------------------------------------------------------------------
Namespace MUSTER.Info
    Public Class CompanyLicenseeCollection
        Inherits DictionaryBase
#Region "Public Events"
        Public Event CompanyLicenseeColChanged(ByVal bolValue As Boolean)
#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.CompanyLicenseeInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.CompanyLicenseeInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.CompanyLicenseeInfo)
                If Not MyBase.Dictionary.Contains(index) Then
                    ...
                    RaiseEvent CompanyLicenseeColChanged(True)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.CompanyLicenseeInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.CompanyLicenseeInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.CompanyLicenseeInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.CompanyLicenseeInfo)) Then
                Throw New ArgumentException("Only CompanyLicensees may be inserted into a CompanyLicensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnRemove(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.CompanyLicenseeInfo)) Then
                Throw New ArgumentException("Only CompanyLicensees may be removed from a CompanyLicensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            If Not newvalue.GetType() Is (GetType(MUSTER.Info.CompanyLicenseeInfo)) Then
                Throw New ArgumentException("Only CompanyLicensees updated in a CompanyLicensee collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnValidate(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.CompanyLicenseeInfo)) Then
                Throw New ArgumentException("Only CompanyLicensees may be validated in a CompanyLicensee collection!", "value")
            End If
        End Sub
#End Region
    End Class
