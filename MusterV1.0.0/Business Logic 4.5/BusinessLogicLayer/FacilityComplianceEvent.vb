...
#Region "Miscellaneous Operations"
        Public Function GetInspections(Optional ByVal facility_id As Integer = 0, Optional ByVal managerID As Integer = Nothing) As DataSet
            ...
            End Try
        End Function
        Public Function GetAdminUSersList() As List(Of String)
            Dim oAdminUsers As List(Of String) = New List(Of String)
            Try
                Dim dsAdminUsers As New DataSet
                dsAdminUsers = oFCEDB.DBGetAdminUsers()
                If dsAdminUsers IsNot Nothing And dsAdminUsers.Tables(0) IsNot Nothing And dsAdminUsers.Tables(0).Rows.Count > 0 Then
                    For i As Integer = 0 To dsAdminUsers.Tables(0).Rows.Count - 1
                        oAdminUsers.Add(dsAdminUsers.Tables(0).Rows(i)("USER_ID").ToString())
                    Next
                End If
            Catch ex As Exception
                If InStr(UCase(ex.Source), UCase("MUSTER.BusinessLogic")) Then MusterException.Publish(ex, Nothing, Nothing)
