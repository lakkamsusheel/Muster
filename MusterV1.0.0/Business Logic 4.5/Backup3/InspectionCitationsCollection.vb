'-------------------------------------------------------------------------------
' MUSTER.Info. InspectionCitationsCollection
...
Namespace MUSTER.Info
    Public Class InspectionCitationsCollection
        Inherits DictionaryBase
        '#Region "Public Events"
        '        Public Event evtInspectionCitationColChanged(ByVal bolValue As Boolean)
        '#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.InspectionCitationInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.InspectionCitationInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.InspectionCitationInfo)
                If Not MyBase.Dictionary.Contains(Integer.Parse(index)) Then
                    ...
                    'RaiseEvent evtInspectionCitationColChanged(True)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.InspectionCitationInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.InspectionCitationInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.InspectionCitationInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCitationInfo)) Then
                Throw New ArgumentException("Only  InspectionCitations may be inserted into a  InspectionCitation collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnRemove(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCitationInfo)) Then
                Throw New ArgumentException("Only  InspectionCitations may be removed from a  InspectionCitation collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            If Not newvalue.GetType() Is (GetType(MUSTER.Info.InspectionCitationInfo)) Then
                Throw New ArgumentException("Only  InspectionCitations updated in a  InspectionCitation collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnValidate(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.InspectionCitationInfo)) Then
                Throw New ArgumentException("Only  InspectionCitations may be validated in a  InspectionCitation collection!", "value")
            End If
        End Sub
#End Region
    End Class
