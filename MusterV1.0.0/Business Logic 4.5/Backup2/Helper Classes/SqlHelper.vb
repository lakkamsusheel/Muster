...
'
' Modified 2/10/2005 - JVC2 - Added GetSpParameterCol() and overloads.  Functions identically to 
'                               GetSpParameterSet() but returns a collection instead of an array.
'
' Release   Initials    Date        Description
'  1.1        AB        02/14/05    Extended ExecuteDataset() & ExecuteReader() to accept parameter collections
'  1.2        MNR       03/08/05    Added try catch in Private Overloads Shared Function ExecuteReader
'  1.3        JVC       08/11/05    Extended ExecuteDataSet() to accept a connection string and parameter collection.
'                                       (Sorry Aaron - wasn't there!)
'  1.4 Thomas Franey    04/07/09    extended Timeout to allo wlarge SQL processes and database restoring 
' ---------------------------------------------------------------------------------------
...
#Region "ExecuteNonQuery"
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connectionString As String, _
                                                     ByVal commandType As CommandType, _
                                                     ByVal commandText As String, _
                                                     ByVal sqlParams As Collection) As Integer
        'create & open a SqlConnection, and dispose of it after we are done.
        Dim commandParameters(sqlParams.Count - 1) As SqlParameter
        Dim oSqlParam As SqlParameter
        Dim nIndex As Integer = 0
        Dim cn As New SqlConnection(connectionString)
        Try
            For Each oSqlParam In sqlParams
                commandParameters(nIndex) = New SqlParameter(oSqlParam.ParameterName, _
                                                             oSqlParam.SqlDbType, _
                                                             oSqlParam.Size, _
                                                             oSqlParam.Direction, _
                                                             True, _
                                                             oSqlParam.Precision, _
                                                             oSqlParam.Scale, _
                                                             oSqlParam.SourceColumn, _
                                                             oSqlParam.SourceVersion, _
                                                             oSqlParam.Value)
                nIndex += 1
            Next
            cn.Open()
    ...
    Public Overloads Shared Function SaveDataTable(ByVal connectionString As String, ByVal dtData As DataTable, Optional ByVal oInsCmd As SqlCommand = Nothing, Optional ByVal oUpdCmd As SqlCommand = Nothing, Optional ByVal oDelCmd As SqlCommand = Nothing) As Boolean
        Dim oAdpt As New SqlClient.SqlDataAdapter
        Dim bSaveData As Boolean = False
        Dim oSqlConnection As New SqlConnection(connectionString)
        Try
            If Not IsNothing(oInsCmd) Then
                oAdpt.InsertCommand = oInsCmd
                oAdpt.InsertCommand.Connection = oSqlConnection
                bSaveData = True
            End If

            If Not IsNothing(oUpdCmd) Then
                oAdpt.UpdateCommand = oUpdCmd
                oAdpt.UpdateCommand.Connection = oSqlConnection
                bSaveData = True
            End If

            If Not IsNothing(oDelCmd) Then
                oAdpt.DeleteCommand = oDelCmd
                oAdpt.DeleteCommand.Connection = oSqlConnection
                bSaveData = True
            End If
            If bSaveData Then
                oAdpt.Update(dtData)
...
#Region "ExecuteDataset"
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connectionString As String, _
        ...
        '
        ' Note - altered JVC 2 11/23/04 - was not resolving properly with original call, so mimiced the other calls.
        '
        Dim cn As New SqlConnection(connectionString)
        Try
            cn.Open()
            ...
            'call the overload that takes a connection instead of a connection string
            ' Original line follows
            'Return ExecuteDataset(connectionString, commandType, commandText, CType(Nothing, SqlParameter())

            Return ExecuteDataset(cn, commandType, commandText) 'CType(Nothing, SqlParameter())
        Catch ex As Exception
            Throw ex
        Finally
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connectionString As String, _
                                                    ByVal commandType As CommandType, _
                                                    ByVal commandText As String, _
                                                    ByVal Params As Collection) As DataSet
        'create & open a SqlConnection, and dispose of it after we are done.
        Dim cn As New SqlConnection(connectionString)
        Try
            cn.Open()
            ...
            'call the overload that takes a connection in place of the connection string
            Return ExecuteDataset(cn, commandType, commandText, Params)
        Finally
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connection As SqlConnection, _
                                                    ByVal commandType As CommandType, _
                                                    ByVal commandText As String, _
                                                    ByVal sqlParams As Collection) As DataSet
...
#Region "ExecuteReader"
    ...
    ' -commandType - the CommandType (stored procedure, text, etc.)
    ' -commandText - the stored procedure name or T-SQL command 
    ' -sqlParams - a collection of SqlParamters used to execute the command 
    ' Returns: a SqlDataReader containing the resultset generated by the command 
    Public Overloads Shared Function ExecuteReader(ByVal connectionString As String, _
                                                         ByVal commandType As CommandType, _
                                                         ByVal commandText As String, _
                                                         ByVal sqlParams As Collection) As SqlDataReader
...
#Region "ExecuteScalar"
    ...
    Public Shared Function GetFunctionalityApprovalFromDB(ByVal connectionString As String, _
        ...
        Try
            ...
            'call the overload that takes a connection in place of the connection string
            Return ExecuteScalar(cn, CommandType.Text, String.Format("select dbo.udfFunctionalityAllowed('{0}')", FunctionName))
        Catch ex As Exception
            Return False
        Finally
            If Not cn.State = ConnectionState.Closed Then
...
#End Region
#Region "Security"
    ' Checks if user has access to save the entity from a given module
    ' Returns: true / false
    Public Shared Function HasWriteAccess(ByVal moduleID As Integer, ByVal staffID As Integer, ByVal entityType As Integer) As Boolean
        ...
        Try
            strSQL = "spGetWriteAccess"

            Params = SqlHelperParameterCache.GetSpParameterCol(oCnn.cnString, strSQL)
            Params("@STAFF_ID").Value = staffID
            Params("@MODULE_ID").Value = moduleID
            Params("@ENTITY_TYPE").Value = entityType
            Params("@DELETED").Value = False

            drSet = ExecuteReader(oCnn.cnString, CommandType.StoredProcedure, strSQL, Params)
            If drSet.HasRows Then
                drSet.Read()
                retVal = drSet.Item("WRITE_ACCESS")
            End If
            Return retVal
        Catch ex As Exception
            Throw ex
...
#Region "Property Master"
    Public Shared Function AddBlankPropertyName(ByVal propertyTypeID As Int16) As String
        Return "SELECT 0 AS PROPERTY_ID, " + propertyTypeID.ToString + " AS PROPERTY_TYPE_ID, '' AS PROPERTY_NAME, '' AS PROPERTY_DESCRIPTION, " + _
                "0 AS PROPERTY_POSITION, 1 AS BUSINESS_TAG, 'YES' AS PROPERTY_ACTIVE, " + _
                "'sa' AS CREATED_BY, GETDATE() AS CREATE_DATE, 'sa' AS LAST_EDITED_BY, GETDATE() AS DATE_LAST_EDITED"
    End Function
#End Region
End Class 'SqlHelper
...
#Region "Parameter Discovery Functions"
    ...
    ' -spName - the name of the stored procedure 
    ' Returns: an array of SqlParameters
    Public Overloads Shared Function GetSpParameterCol(ByVal connectionString As String, ByVal spName As String) As Collection
        Return GetSpParameterCol(connectionString, spName, False)
    End Function 'GetSpParameterSet 
    ...
    ' -includeReturnValueParameter - a bool value indicating whether the return value parameter should be included in the results 
    ' Returns: an array of SqlParameters 
    Public Overloads Shared Function GetSpParameterCol(ByVal connectionString As String, _
                                                       ByVal spName As String, _
                                                       ByVal includeReturnValueParameter As Boolean) As Collection
        ...
        For Each oSqlParam In cachedParameters
            colParams.Add(oSqlParam, oSqlParam.ParameterName)
        Next
