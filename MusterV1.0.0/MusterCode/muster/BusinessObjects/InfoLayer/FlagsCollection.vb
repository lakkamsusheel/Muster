...
'  1.3        JVC2      01/31/2005  Modified ITEM to use value.id.tostring for key
'
...
' Remove(key)       Removes the key supplied from the Entity Collection
'
'-------------------------------------------------------------------------------
...
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.FlagInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.FlagInfo)
            End Get
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.FlagInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.FlagInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.FlagInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.FlagInfo)) Then
                Throw New ArgumentException("Only Flag may be inserted into a Flag collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnRemove(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.FlagInfo)) Then
                Throw New ArgumentException("Only Flag may be removed from a Flag collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            If Not newvalue.GetType() Is (GetType(MUSTER.Info.FlagInfo)) Then
                Throw New ArgumentException("Only Flags updated in a Flag collection!", "value")
            End If
        End Sub

        Protected Overloads Sub OnValidate(ByVal Key As Object, ByVal value As Object)
            If Not value.GetType() Is (GetType(MUSTER.Info.FlagInfo)) Then
                Throw New ArgumentException("Only Flags may be validated in a Flag collection!", "value")
            End If
        End Sub
#End Region
    End Class
