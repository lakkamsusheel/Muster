...
Namespace MUSTER.Info
    ' -------------------------------------------------------------------------------
    ' MUSTER.Info.PendFeeLineCollection
    '       Provides a stongly-typed collection for storing Pending Fee Line objects
    ' 
    ...
    ' -------------------------------------------------------------------------------
    Public Class PendFeeLineCollection
        '
        Inherits DictionaryBase
#Region "Public Events"
        Public Event PendFeeLineColChanged(ByVal strSrc As String)
#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.PendFeeLineInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.PendFeeLineInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.PendFeeLineInfo)
                If MyBase.Dictionary.Contains(index) Then
                    MyBase.Dictionary.Item(index) = Value
                    RaiseEvent PendFeeLineColChanged(Me.ToString)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.PendFeeLineInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.PendFeeLineInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.PendFeeLineInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            'If Not value.GetType() Is (GetType(MUSTER.Info.PendFeeLineInfo)) Then
        ...
        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            'If Not newvalue.GetType() Is (GetType(MUSTER.Info.PendFeeLineInfo)) Then
