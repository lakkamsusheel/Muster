...
Namespace MUSTER.Info
    ' -------------------------------------------------------------------------------
    ' MUSTER.Info.FeeLateFeeCollection
    '       Provides a stongly-typed collection for storing Late Fee objects
    ' 
    ...
    ' -------------------------------------------------------------------------------
    Public Class FeeLateFeeCollection
        '
        Inherits DictionaryBase
#Region "Public Events"
        Public Event FeeLateFeeColChanged(ByVal strSrc As String)
#End Region
#Region "Exposed Operations"
        Default Public Property Item(ByVal index As String) As MUSTER.Info.FeeLateFeeInfo
            Get
                Return CType(MyBase.Dictionary.Item(index), MUSTER.Info.FeeLateFeeInfo)
            End Get
            Set(ByVal Value As MUSTER.Info.FeeLateFeeInfo)
                If MyBase.Dictionary.Contains(index) Then
                    MyBase.Dictionary.Item(index) = Value
                    RaiseEvent FeeLateFeeColChanged(Me.ToString)
                Else
        ...
        Public ReadOnly Property GetKeys() As String()
            ...
            End Get
        End Property
        Public Sub Add(ByVal value As MUSTER.Info.FeeLateFeeInfo)
            Me.Item(value.ID.ToString) = value
        End Sub
        Public Function Contains(ByVal value As MUSTER.Info.FeeLateFeeInfo) As Boolean
            Return MyBase.Dictionary.Contains(value.ID.ToString)
        End Function
        Public Function Contains(ByVal Key As String) As Boolean
            Return MyBase.Dictionary.Contains(Key)
        End Function
        Public Sub Remove(ByVal value As MUSTER.Info.FeeLateFeeInfo)
...
#Region "Overloaded Operators"
        Protected Overloads Sub OnInsert(ByVal Key As Object, ByVal value As Object)
            'If Not value.GetType() Is (GetType(MUSTER.Info.FeeLateFeeInfo)) Then
        ...
        Protected Overloads Sub OnSet(ByVal Key As Object, ByVal oldvalue As Object, ByVal newvalue As Object)
            'If Not newvalue.GetType() Is (GetType(MUSTER.Info.FeeLateFeeInfo)) Then
